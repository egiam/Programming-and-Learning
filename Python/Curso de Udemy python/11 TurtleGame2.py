...
s = turtle.Screen()
s.setup(650,650)   #tamanio de pantalla
s.bgcolor("black")
s.title("Proyecto 2")
...
def movimiento():
    if serpiente.direction == 'up':
      y = serpiente.ycor()  #devuelve la cordenada en Y
      serpiente.sety(y + 20) 
    if serpiente.direction == 'down':
        y = serpiente.ycor()  #devuelve la cordenada en Y
        serpiente.sety(y - 20) 
    if serpiente.direction == 'right':
        x = serpiente.xcor()  #devuelve la cordenada en X
        serpiente.setx(x + 20) 
...
s.listen()  #'Escuchar' el teclado
s.onkeypress(arriba, "Up")      #Para q se gire con las flechas
...
while True:
    ...
    if serpiente.distance(comida) < 20:     #.distance = la distancia q tiene contra cierto objeto
    ...
    total = len(cuerpo)
    for index in range(-1,0,-1):         #-1 para q la cuenta incie hacia atras, 0 para q no se cuente, 1 para q valla de uno en uno
